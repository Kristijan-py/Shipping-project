...
export const handleGoogleOAuth = async (accessToken, refreshToken, profile, cb) => {
    try {
      ...
      if (user.length === 0) { // USER DOES NOT EXIST
        // Check again with email in users table(if user signed up manually)
        let userId;
        ...
        if(existingEmail.length === 0){
          ...
          userId = userResult; // if  not existed, add the new id from created user
...
// It handles the Facebook OAuth process
export const handleFacebookOAuth = async (accessToken, refreshToken, profile, cb) => {
    try {

      const user = await findOAuthUser('facebook', profile.id);
      if (user.length === 0) { // USER DOES NOT EXIST
        ...
        const insertResult = await createUserInOAuthTable(userId, 'facebook', profile.id);
        if (!insertResult) {
          return cb(new Error('Failed to create OAuth user in the database'));
        }
        const user = {
...
// It handles the callback after Facebook OAuth
